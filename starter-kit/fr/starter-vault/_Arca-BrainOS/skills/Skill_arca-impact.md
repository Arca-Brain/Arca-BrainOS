...
1. **Recherche de Contexte (Projets & Thèmes liés) :**
   - Scanne le dossier `1-Projects/` à la recherche des projets actifs (`P-[Nom].md`) ou des fichiers thématiques (`T-[Nom].md`).
...
2. **Élaboration du Diagnostic d'Impact :**
   ...
   - Formule des recommandations structurées selon 3 axes :
     ...
     - **🧠 Connaissances à intégrer :** Suggestions d'ajouts ou d'ajustements dans vos notes de synthèse ou de concepts.

3. **Phase Interactive (Validation) :**
   - Présente le diagnostic de manière claire et structurée à l'utilisateur dans le chat.
   - Propose des choix d'actions précis (ex: *"Voulez-vous que j'ajoute la tâche [X] dans le projet [[P-Finance-Perso]] ?"*).
...
4. **Application & Journalisation :**
   - Applique chirurgicalement les modifications validées par l'utilisateur sur les projets (en respectant le style humain).
   - **Enregistrement de l'Impact :** Pour chaque action validée et appliquée sur un projet (ex: `[[P-Nom]]`), écris une ligne récapitulative dans la section `## Application & Impact` de la note distillée source d'origine sous le format : `- [[P-Nom-Projet]] : Tâche "Description de la tâche" ajoutée le YYYY-MM-DD.` (si aucune action n'est validée, écris : `Aucune action directe planifiée à ce jour.`).
